...
Because PCA is purely a math equation, it will instantly crash if it tries to read text columns like "Overweight" or "Female." You have to temporarily hide those text categories, scale the data, and run PCA only on the numbers.

Once PCA hands you back the new components (PC 1 and PC 2), you bring your text categories back out of hiding and use them to color-code the scatterplot. It's basically a blind taste test—you are checking to see if the algorithm naturally separated the groups without ever being told what the groups were!
