...
*   Calculate the rolling mean of `all_scores` using a window size of 10.
*   Plot the results.

```python
# Visualize the scores
ax = all_scores.____(10).____().plot(cmap=plt.cm.coolwarm)
ax.set(title='Scores for multiple window sizes', ylabel='Pearson R')
plt.show()
```
