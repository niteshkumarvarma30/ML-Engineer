...
plt.plot(____, color='k', lw=3, label='Actual')
plt.plot(____, color='r', lw=2, label='Predicted')
plt.legend()
plt.show()
```
...
```python
# Visualize our predictions just like we did with our original data
plt.plot(y_test, color='k', lw=3, label='Actual')
plt.plot(predictions, color='r', lw=2, label='Predicted')
plt.legend()
plt.show()
```
